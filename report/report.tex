...
A particle filter is used to localize the location and orientation of the
robot.  Measurements of the distance, orientation and heading are done by the
phone's camera using object tracking. The accelerometer sensor is used to
detect walking activity, with a kNN classifier, and user orientation, which
updates the particles of the filter. The robot is controlled by a \texttt{IOIO}
board, a simple hardware device that directly follows commands from the phone
through a Bluetooth connection. We found that the localization with the
particle filter is working well. Measuring the distance with the camera works
well as well, but only for smaller distances.
