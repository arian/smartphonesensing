...
camera. The accelerometer sensor is used to detect walking activity and user
orientation, which updates the particles of the filter. The robot is controlled
by a \texttt{IOIO} board, a simple hardware device that directly follows
commands from the phone through a Bluetooth connection. We have found that it is awesome
